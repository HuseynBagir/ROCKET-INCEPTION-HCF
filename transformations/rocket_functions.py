...
@jitclass([('pooling', string)])
class rock:
    def __init__(self, pooling):
        self.pooling = pooling
    
    @staticmethod
    @njit("Tuple((float64[:],int32[:],float64[:],int32[:],int32[:]))(rock, int64,int64)")
    def generate_kernels(self, input_length, num_kernels):
    ...
    def apply_kernels(self, X, kernels):
        ...
        for i in prange(num_examples):
            ...
            for j in range(num_kernels):
                ...
                _X[i, a2:b2] = \
                rock.apply_kernel(X[i], weights[a1:b1], lengths[j], biases[j], dilations[j], paddings[j])
